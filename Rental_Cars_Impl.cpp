...
void MyRentalCars::VehicleType()
{
	...
	cout << "Are you planning to rent a car, or truck? ";
	cin >> Vehicle_Type;
	//we need to check if the user enters truck or car.
	//loop through a statement while the users input is not equal to Car,car,Truck,truck.
	while ((Vehicle_Type != "Car") && (Vehicle_Type != "car") && (Vehicle_Type != "Truck") && (Vehicle_Type != "truck"))
	{
		cout << "Please enter Truck or Car: ";
		cin >> Vehicle_Type;
		/*if ((Vehicle_Type == "Car" || Vehicle_Type == "car") || (Vehicle_Type == "Truck" || Vehicle_Type == "truck"))
		{
			break;
		}*/
	}
...
void MyRentalCars::DaysOfRental()
{
	...
	while (RentalDays <= 0)
	{
		cout << "how many days are you planning to rent the vehicle, days can't be 0 or less than 0? ";
		cin >> RentalDays;
...
{
	cout << "Enter the corrisponding vehicle brand # you would like to rent? ";
	cin >> Vehicle_Brand_Chose;
	while (Vehicle_Brand_Chose <= 0)
	{
		//use recursion
		ShowRentalMenu();
		cout << "Enter the corrisponding vehicle brand # you would like to rent? ";
	...
	//check if the user enters a correct corresponding vehicle brand number.
	while ((Vehicle_Brand_Chose != 1) && (Vehicle_Brand_Chose != 2) && (Vehicle_Brand_Chose != 3) && (Vehicle_Brand_Chose != 4) && (Vehicle_Brand_Chose != 5))
	{
		//use recursion
		ShowRentalMenu();
		cout << "Please enter the number corrisponding to the brand of vehicle you would like to rent. ";
		cin >> Vehicle_Brand_Chose;
		Vehicle_Selection = CarBrands[Vehicle_Brand_Chose - 1];
